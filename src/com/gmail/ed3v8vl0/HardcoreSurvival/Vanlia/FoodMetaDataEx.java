...
public class FoodMetaDataEx extends FoodMetaData {
	...
	private int e = 20; //prevFoodLevel

    public FoodMetaDataEx() {
    	super();
    }
    
    public FoodMetaDataEx(EntityHuman entityhuman) {
    	super(entityhuman);
    ...
    public void a(ItemFood itemfood, ItemStack itemstack) {
        ...
        FoodLevelChangeEvent event = CraftEventFactory.callFoodLevelChangeEvent(entityhuman, itemfood.getNutrition(itemstack) + oldFoodLevel);
    ...
    public void a(EntityHuman entityhuman) {
        ...
        double hungry = SurvivalCalc.hungrySpeed;
        
        this.e = this.foodLevel;
        if (this.exhaustionLevel > hungry) {
            this.exhaustionLevel -= hungry;
            if (this.saturationLevel > 0.0F) {
                this.saturationLevel = Math.max(this.saturationLevel - 1.0F, 0.0F);
            } else if (enumdifficulty != EnumDifficulty.PEACEFUL) {
            	entityhuman.sendMessage(new ChatComponentText("[DEBUG] FoodMetaDataEx"));
                // CraftBukkit start
